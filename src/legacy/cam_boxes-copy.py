...
class Box:
    ...
    def __init__(self, startX=None, startY=None, endX=None, endY=None):
        ...
        if coordinate_str is not None:  # init from string, usually - part of file name
            ...
            self.startX, self.startY, self.endX, self.endY = Box.sides_2_corners()
...
def test2():
    ...
    (Box().union(b2).union(b1)).draw2(img, BGR_YELLOW, '        uu')
    (Box().intersect(b2).intersect(b1)).draw2(img, BGR_CHOCOLATE, '   ii')
    cv2.imshow('test',img)
